...
async def get_comprehensive_analysis(
    ...
    try:
        ...
        try:
            ...
            from hydrological_analysis_system import WatershedWaterBalance
        except ImportError as e:
            logger.error(f"水文分析模块导入失败: {e}")
            raise HTTPException(status_code=503, detail="水文分析模块不可用")
        ...
        return {
            ...
            "recommendations": {
                "data_quality": "Data from pipeline sync" if df is not None else "Static data (may be outdated)",
                "analysis_confidence": "High" if not balance_results.get('error') else "Low",
                "next_steps": [
                    "Monitor water balance trends",
                    "Validate precipitation data sources",
                    "Update analysis with fresh data"
                ]
            },
            "data_source": os.path.basename(data_path),
            "analysis_type": "comprehensive_hydrological",
...
async def get_flood_frequency_analysis(
    ...
    try:
        ...
        # 初始化洪水频率分析
        from hydrological_analysis_system import FloodFrequencyAnalysis
...
@router.post("/precipitation-runoff")
async def get_precipitation_runoff_analysis(
    ...
    try:
        ...
        # 初始化降水径流分析
        from hydrological_analysis_system import PrecipitationRunoffAnalysis
