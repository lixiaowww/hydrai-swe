#!/usr/bin/env python3
"""
MODIS data fetcher (real, via direct CMR/LAADS HTTP)
- Cloud-ready: uses Bearer token directly, no .netrc dependency
- Source: NASA CMR + LAADS DAAC

Usage:
  python3 scripts/fetchers/modis_fetch.py --start 2024-01-01 --end 2024-12-31 --bbox -102,48,-95,51 --collection MOD10A1.061 --out data/raw/nasa_modis_snow

Notes:
- Requires EARTHDATA_BEARER env var
- Downloads real MODIS granules via HTTP
"""
...
def download_granules(granules: list, out_dir: str):
    """Download MODIS granules to output directory"""
    token = os.environ.get("EARTHDATA_BEARER")
    headers = {"Authorization": f"Bearer {token}"}
...
def main():
    ...
    parser.add_argument("--collection", default="MOD10A1.061")
    ...
    try:
        # Search CMR
        granules = search_cmr(args.collection, args.start, args.end, bbox, args.limit)
        if not granules:
            return 3
